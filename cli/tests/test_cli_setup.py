...
_ALL_FOUND = {"fusion": "/stub/fusion", "git": "/stub/git",
              "soffice": "/stub/soffice", "uv": "/stub/uv"}
...
def sandbox(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(setup.shutil, "which",
                         lambda name, *a, **kw: _ALL_FOUND.get(name))
...
def test_setup_advice_runs_update_shell_when_fusion_off_path(
    ...
    monkeypatch.setattr(setup.shutil, "which",
                         lambda name, *a, **kw: which_map.get(name))
    ...
    assert any(a["topic"] == "path" and "restart" in a["text"]
               for a in out["advice"])
...
def test_setup_advice_respects_no_modify_path(sandbox, capsys, monkeypatch):
    ...
    assert calls == []
    assert any(a["topic"] == "path" and "yourself" in a["text"]
               for a in out["advice"])
